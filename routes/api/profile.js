...
router.get("/all", (req, res) => {
    ...
    Profile.find()
        ...
        .then((profiles) => {
            if (!profiles) {
                errors.noprofile = "There is no profile to view.";
                return res.status(404).json(errors);
            }
            res.json(profiles);
        })
        ...
        Profile.findOne({ user: req.user.id }).then((profile) => {
            ...
            if (profile) {
                ...
                Profile.findOne({ handle: profileFields.handle }).then(
                    (profile) => {
                        if (profile) {
                            errors.handle = "The handle is already taken.";
            ...
            .then((profile) => {
                ...
                if (removeIndex !== -1) {
                    ...
                    return res.status(404).json({
                        error: "Cannot find experience with given ID.",
                    });
                }
            })
            .catch((err) => res.status(404).json(err));
        ...
        Profile.findOne({ user: req.user.id })
            .then((profile) => {
                ...
                if (removeIndex !== -1) {
                    ...
                    return res.status(404).json({
                        error: "Cannot find education with given ID.",
                    });
                }
            })
            .catch((err) => res.status(404).json(err));
